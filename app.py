...
class noForm(FlaskForm):
    no = IntegerField(
        'how many recent days do you want the graph to cover?', validators=[DataRequired()])
...
class DataHandler(object):
    ...
    def update(self):
        ...
        self.available = [i for i in cursor.execute(
            'select * from datas order by date')]
        ...
        # if today is weekend. and this weeks' data has been updated. then
        # don't bother.
        elif now.weekday() in [5, 6] and now.date() - most_recent_data.date() < timedelta(3) and most_recent_data.weekday() == 4:
            print('Done with this week.')

        # if today is monday and last friday has been updated.and not up to 3
        # pm .
        elif now < today3pm and now.weekday() == 0 and most_recent_data.date() == now.date() - timedelta(3):
            print('not 3pm yet')
            return
        # if today is weekday. and yesterday's updated. and now is not up to
        # 3pm. don't bother.
            ...
            for i in shzdict:
                try:
                    ...
                    print(e)
            self.today = combined
    # check from last element if info is already available. if not push new
    # item. if yes stop iteration.
            for i in sorted(combined.keys(), reverse=True):
                datei = datetime.strftime(
                    datetime.strptime(str(i), '%Y%m%d'), '%Y-%m-%d')
                print(datei)
                if datei in self.available_dates:
                    break
                try:
                    cursor.execute(
                        'insert into datas values (?,?)', (datei, combined[i]))
                except BaseException as e:
                    print(e)
